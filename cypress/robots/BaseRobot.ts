...
export abstract class BaseEyes {
    ...
    seesColumnsInTable(...colNames: string[], index: number) {
        const columns[] = cy.get('thead.amino-table__header');
        for(let index in colNames) {
            columns[index].contains(colNames[index])
        }
        return this;
    }
    seesTextInAGridCell(rowclass: string, text: string) {
        cy.get(`[row-id=${rowclass}]`)
            .should('have.text', text);
        return this;
    }
    //
...
export class BaseHands {
    ...
    UploadFile(Filepath:string, file: string){
        const FixturePath = file;
         cy.get(Filepath).attachFile(FixturePath, { force: true });
    }
...
export class BaseDependencies {
    ...
    login() {
        cy.get('input[type="email"]').type(Cypress.env('USER_NAME'));
        cy.get('input[type="password"]').type(Cypress.env('PASSWORD'));
        // cy.get('form').submit() // Not ideal. We will ensure we click the button
